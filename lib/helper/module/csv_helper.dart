...
class CsvHelper {
  ...
  Future<void> saveToCsvAngle(List<double> w,List<double> x,List<double> y,List<double> z, List<DateTime> timestamp) async {
    // 데이터 검증
    if (w.length != x.length || w.length != y.length || w.length != z.length) {
      throw Exception("pitch, yaw, roll 데이터의 길이가 같아야 합니다.");
    }
  ...
  Future<void> saveVectorsToCsv(List<Vector3> vectors, List<DateTime> timestamp) async {
    // 데이터 검증
    // if (w.length != x.length || w.length != y.length || w.length != z.length) {
    //   throw Exception("pitch, yaw, roll 데이터의 길이가 같아야 합니다.");
    // }
    ...
    for (int i = 0; i < vectors.length; i++) {
      rows.add([ vectors[i].x, vectors[i].y,vectors[i].z,timestamp[i].millisecondsSinceEpoch]);
    }
    ...
    // 파일 저장
    final directory = await getExternalStorageDirectory();
    final path = "${directory?.path}/vector_${DateTime.now().millisecondsSinceEpoch}.csv";
    final file = File(path);
